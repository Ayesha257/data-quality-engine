"""
Centralized logging for the Data Quality Engine.

Two complementary JSONL run-log mechanisms (behavior preserved from the
former split between engine/logging_utils.py and database/logging_setup.py):

    - Pipeline run logs  -> logs/run_{run_id}.jsonl
      Used by Phase 1 checks and main.run_pipeline().

    - Phase 2 audit logs -> logs/phase2_run_{run_id}.jsonl
      Used by API/services; includes client_id on each event.

Run manifests (logs/manifests/manifest_{run_id}.json) are written alongside
Phase 2 audit logs for compliance/audit snapshots.
"""
...
# ---------------------------------------------------------------------------
# Phase 1 pipeline JSONL (logs/run_{run_id}.jsonl)
# ---------------------------------------------------------------------------
